...
2. **Apply the `dev-workflow:coding-rules` skill.** Key reminders: small single-responsibility functions with explicit names, extract on the third duplication (not the first), match the surrounding code's style and idioms.
...
5. **Never commit or push unless the brief explicitly asks.** If it does, follow the `dev-workflow:feature-workflow` skill: Conventional Commits, and never any Claude/AI attribution.
